...
if __name__ == "__main__":
    ...
    Pa = np.array(df[['intercept']+["P%d" %(i+1) for i in range(PCA_COMPS)]])
    T = np.array(df[['type0', 'type1', 'type2', 'type3', 'type4', 'type5']])
    W = np.dot(np.linalg.pinv(Pa), T)
    df['type_pred'] = calcMultiLinPred(Pa, W)
    type_cm = confusion_matrix(df['Y'], df['type_pred'])
    
    # Utilize classifier to get the name of a photo
    # Apply PCA to the test set
    Z_test = X_test - M
    C_test = np.cov(Z_test, rowvar=False)
    eigvals_test, V_test = np.linalg.eigh(C_test)
    eigvals_test=np.flipud(eigvals_test)
    V_test=np.flipud(V_test.T)
    V_test = V_test[:PCA_COMPS]
    P_test = np.dot(Z_test,V_test.T)
    
    # Resulting data frame
    intercept = np.array([1 for i in range(np.alen(Y_test))]).reshape((np.alen(Y_test), 1))
    df_test = pd.DataFrame(np.concatenate((intercept, P_test), axis=1)) 
    df_test.columns = ['intercept']+["P%d" %(i+1) for i in range(PCA_COMPS)]
    Pa_test = np.array(df_test[['intercept']+["P%d" %(i+1) for i in range(PCA_COMPS)]])
    df_test['type_pred'] = calcMultiLinPred(Pa_test, W) # Get predictions
    
    # Get the most common class of the n test samples
    type_preds = list(df_test['type_pred'])
    counts = [type_preds.count(x) for x in set(type_preds)]
    preds = [x for x in set(type_preds)]
    idx, val = max(enumerate(counts), key=operator.itemgetter(1))
    PREDICTION_CLASS = preds[idx]
